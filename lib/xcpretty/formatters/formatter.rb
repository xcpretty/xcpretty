...
module XCPretty
  ...
  module FormatMethods
    ...
    def format_analyze(file_name, file_path);                     end
    ...
    def format_codesign(file);                                    end
    def format_compile(path);                                     end
    def format_compile_command(compiler_command, file_path);      end
    def format_compile_storyboard(file_name, file_path);          end
    def format_compile_swift_sources();                           end
    def format_compile_xib(file_name, file_path);                 end
    ...
    def format_failing_test(suite, test, time, file_path);        end
    def format_generate_dsym(dsym);                               end
    def format_libtool(library);                                  end
    def format_linking(file, build_variant, arch);                end
    ...
    def format_process_pch_command(file_path);                    end
    ...
    def format_touch(file_path, file_name);                       end
    ...
    def format_file_missing_error(error, file_path);                   end
    def format_ld_warning(message);                                    end
    def format_undefined_symbols(message, symbol, reference);          end
    def format_duplicate_symbols(message, file_paths);                 end
